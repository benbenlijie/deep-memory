...
<p align="center"><em>它只记住你批准留下的东西。其他东西，先证明自己值得被记住。</em></p>

<div align="center">

  <p><strong>给那些该记的不记、不该记的乱记、还很少先问你的 Agent，一个本地记忆本。</strong></p>

  <p>
    <code>deep-memory</code> 是我在把真实 Agent 接到一起时想要的 memory layer：本机上很无聊、但可靠的 SQLite，
    显式 durable facts 和 reusable procedures，有 scope 的召回，以及一个真的能删除东西的删除按钮。
    没有隐藏云状态，不囤 raw transcript，也不搞那种只能相信 UI 文案的神秘“记忆”。
  </p>
...
你应该见过这种场景：Agent 很自信地忘了你昨天刚说过的规则。然后，它又不知道从哪儿捞出三次实验前一条半错的偏好，塞进一个完全不相关的新 repo。

`deep-memory` 是给那些喜欢 Agent、但不想让 Agent memory 变成玄学的人用的。
...
没有共享记忆层时，每个 Agent 都有同一个坏习惯：昨天刚解释过的东西它今天忘了；某些你不想让它长期带着的东西，它又不知道藏在哪儿记住了。你反复解释偏好、仓库约定、安全规则，还有那些“下次别再这样”的小纠偏。过一周再看，还是不知道到底哪些东西留下来了。

有了 `deep-memory`，一条有用约定会变成有 scope 的、可审查 record：

```text
project:deep-memory  procedural  "review 前运行 uv run pytest -q"
user:ben             semantic    "用户偏好：中文为主，技术术语用英文"
```

Claude Code、Codex、OpenCode、Hermes 可以在任务开始前取回同一段有界上下文。你也可以通过 CLI 或本地 WebUI 检查、编辑、导出、软删除，必要时 hard-delete，让一条坏记忆别再继续带偏未来行为。

## 为什么需要它

我不想让 Agent 通过悄悄积累不可见状态来变得更“个性化”。那只是给 bug 换了个更难找的地方。

我想要的 memory 更像基础设施：默认本地；感觉不对时能检查；scope 足够窄，不把一个项目的上下文带去另一个项目；并且能被测试到——“支持中文”应该意味着 eval 跑过，而不是 README 上贴了个标签。
...
- **本地治理。** 一份本机 SQLite 数据库供多个 Agent 共享，再用固定 `scope` 和自定义 `scope_id` 把记录限制在 global、user、tenant、workspace、project 命名空间内。
...
Agent 在依赖记忆之前，先走一条很短的治理路径：

```text
1. 这是否长期有效？       → 否：不写入
2. 是否适合保留？         → secrets / raw private transcripts / 临时状态直接拒绝
3. 最窄 scope 是什么？    → project、workspace、user、tenant 或 global
4. 人能否检查？           → CLI、Python SDK、本地 WebUI、export、audit
5. 它是否仍然有用？       → edit、deprecate、resolve conflict 或 delete
6. 它是否是流程型记忆？   → 导出为可审查 skill candidate，不自动安装成行为规则
```
...
```text
$ uv run deep-memory search ~/.deep-memory/deep-memory.db "共享 deep-memory 持久记忆" --scope project --scope-id deep-memory
score=...  scope=project  scope_id=deep-memory  kind=semantic  content=用户希望多个 Agent 共享 deep-memory 持久记忆
```
...
数据库可以共享，但 retrieval 需要靠 `scope` 和 `scope_id` 保持相关性。`scope` 是固定治理层（`global`、`user`、`tenant`、`workspace`、`project`），`scope_id` 是这一层下面的自定义命名空间，例如 `deep-memory`、`repo-a` 或 `ben`。默认从能满足需求的最窄 scope 开始，只有当某条记忆确实应该跨项目或跨 Agent 流动时，再扩大它的 scope。
...
mem.add("用户偏好：中文为主，技术术语用英文", kind="semantic", importance=0.9, scope="user", scope_id="ben")
mem.add("项目约定：使用 uv 运行测试", kind="procedural", importance=0.8, scope="project", scope_id="deep-memory")

for result in mem.search("这个仓库怎么跑测试？", scope="project", scope_id="deep-memory", limit=3):
...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 本地持久化 | 已实现 | 用户控制的全机本地 SQLite DB，并支持 global/user/tenant/workspace/project 固定 scope 与自定义 scope_id。 |
...
- 默认使用全机本地 SQLite；
- scope 是固定治理层，scope_id 是自定义命名空间；global 记忆要有意写入，project/workspace 记忆要保持边界；
- 每次只取回少量相关上下文；
