<div align="center">
  ...
  <p>
    ...
    再用 user、workspace、project、workflow 等 scope 控制边界——
...
- **本地治理。** 一份本机 SQLite 数据库供多个 Agent 共享，再用显式 scope 把记录限制在 user、workspace、project、workflow 等边界内。
...
- **作用域控制（Scoping）。** 全机级记忆必须通过显式的 user、workspace、project、workflow scope 保持相关性。
