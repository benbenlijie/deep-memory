...
def run_eval(path: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        for row in rows:
            ...
            answer = "\n".join(r.record.content for r in mem.search(row["query"], limit=5))
            ok = all(k in answer for k in row["expected_keywords"])
            passed += int(ok)
            details.append({"id": row["id"], "category": row["category"], "pass": ok})
    return {"task_count": len(rows), "passed": passed, "accuracy": passed / len(rows) if rows else 0, "details": details}
...
def main() -> None:
    ...
    result = run_eval(Path(args.data))
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        print(f"accuracy={result['accuracy']:.3f} passed={result['passed']}/{result['task_count']}")
